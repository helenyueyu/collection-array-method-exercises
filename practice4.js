// Exercise 4
const abbeyRoadRecords = [{song: "Land of Hope and Glory",
...
// Assuming that this uses regular months rather than JS months (since 11 would be December in that case)
const novemberArtists = (array) => {
  return array.filter(x => x.month === 11).map(x => x.artist)
}
// console.log(novemberArtists(abbeyRoadRecords))

const mostFrequent = (array) => {
  let artistObject = array.map(x => x.artist).reduce((acc, curr) => {
  ...
  let sortableArray = []
  for (let artist in artistObject) {
    sortableArray.push([artist, artistObject[artist]])
  }
  sortableArray.sort((a,b) => b[1] - a[1])
  // We are only keeping the artists whose number is equal to the top most recorded artist (this way, we can keep multiple artist if they tie for topmost recording)
  return sortableArray.filter(x => x[1] === sortableArray[0][1]).map(x => x[0])
}
// console.log(mostFrequent(abbeyRoadRecords))

const lastBeatlesSong = (array) => {
  let sortedArray = array.filter(x => x.artist==="The Beatles").sort((a,b) => b.year - a.year)
  return sortedArray.filter(x => x.year === sortedArray[0].year)
}
// console.log(lastBeatlesSong(abbeyRoadRecords))


const sixtiesSong = (array) => {
  return array.filter(x => x.year >= 1960 && x.year <= 1969).sort((a,b) => b.year - a.year).sort((a,b) => b.month - a.month).slice(0,1)
}
